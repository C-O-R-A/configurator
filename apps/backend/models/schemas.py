...
class MotorInterface(BaseModel):
    motor_type:               Literal["shaft", "flat", "hollow"] = "shaft"
...
class JointManifest(BaseModel):
    jid:              str
    joint_type:            Literal["revolute", "prismatic", "continuous", "universal", "spherical", "fixed"]
    displayName:     str
...
class SceneJoint(BaseModel):
    ...
    localPosition:    tuple[float, float, float] = (0.0, 0.0, 0.0)
    localRotation:    tuple[float, float, float] = (0.0, 0.0, 0.0)
...
class GearboxConfig(BaseModel):
    gbid:              str
    gearbox_type:      Literal["planetary", "harmonic", "spur", "wolfrom"]
    ratio:             float
    efficiency:        float
